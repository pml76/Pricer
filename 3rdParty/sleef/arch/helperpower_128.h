...
static INLINE vint vrint_vi_vd(vdouble vd) {
    ...
    return vec_perm(vd, vd, (vector
    unsigned char)(0, 1, 2, 3, 8, 9, 10, 11, 8, 9, 10, 11, 12, 13, 14, 15));
}

static INLINE vint2 vrint_vi2_vf(vfloat vf) { return vec_signed(vec_round(vf)); }

static INLINE vint vtruncate_vi_vd(vdouble vd) {
    return vec_perm(vec_signed(vd), vec_signed(vd), (vector
    unsigned char)(0, 1, 2, 3, 8, 9, 10, 11, 8, 9, 10, 11, 12, 13, 14, 15));
}
...
static INLINE vdouble vsel_vd_vo_vd_vd(vopmask o, vdouble x, vdouble y) {
    return vec_sel(y, x, (vector unsigned long long) o);
}
...
static INLINE int vtestallones_i_vo64(vopmask g) {
    return vec_all_ne(vec_and(g, (vector
    unsigned int)(0, 0, 0xffffffff, 0xffffffff)), (vector
    unsigned int)(0, 0, 0, 0));
}

static INLINE int vtestallones_i_vo32(vopmask g) {
    return vec_all_ne(g, (vector
    unsigned int)(0, 0, 0, 0));
}

static INLINE vopmask vcast_vo32_vo64(vopmask m) {
    return vec_perm(m, m, (vector
    unsigned char)(4, 5, 6, 7, 12, 13, 14, 15, 8, 9, 10, 11, 12, 13, 14, 15));
}
...
static INLINE vdouble vrev21_vd_vd(vdouble vd) {
    return vec_perm(vd, vd, (vector
    unsigned char)(8, 9, 10, 11, 12, 13, 14, 15, 0, 1, 2, 3, 4, 5, 6, 7));
}

static INLINE vdouble vreva2_vd_vd(vdouble vd) { return vd; }

static INLINE vfloat vrev21_vf_vf(vfloat vf) {
    return vec_perm(vf, vf, (vector
    unsigned char)(4, 5, 6, 7, 0, 1, 2, 3, 12, 13, 14, 15, 8, 9, 10, 11));
}

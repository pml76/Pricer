...
static INLINE int vtestallones_i_vo64(vopmask g) {
    int ret = 1;
    for (int i = 0; i < VECTLENDP * 2; i++) ret = ret && g[i];
    return ret;
}
...
static INLINE vdouble vsel_vd_vo_vd_vd(vopmask o, vdouble x, vdouble y) {
    return (vdouble) (((vmask) o & (vmask) x) | ((vmask) y & ~(vmask) o));
}

static INLINE vint2 vsel_vi2_vo_vi2_vi2(vopmask o, vint2 x, vint2 y) {
    return (vint2) (((vmask) o & (vmask) x) | ((vmask) y & ~(vmask) o));
}
...
static INLINE vint vrint_vi_vd(vdouble vd) {
    return vtruncate_vi_vd(vsel_vd_vo_vd_vd((vopmask) (vd < 0.0), vd - 0.5, vd + 0.5));
}
...
static INLINE vfloat vsel_vf_vo_vf_vf(vopmask o, vfloat x, vfloat y) {
    return (vfloat) (((vmask) o & (vmask) x) | (~(vmask) o & (vmask) y));
}
...
static INLINE vint2 vrint_vi2_vf(vfloat vf) {
    return vtruncate_vi2_vf(vsel_vf_vo_vf_vf((vopmask) (vf < 0), vf - 0.5f, vf + 0.5));
}
...
static INLINE vfloat vsqrt_vf_vf(vfloat d) {
    ...
    q = (vfloat) ((o & (vmask) vcast_vf_f(0.00000000023283064365386962890625f)) |
                  (~o & (vmask) vcast_vf_f(1))); // 2^-32
